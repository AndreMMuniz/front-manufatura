...
const SENSITIVE_KEY = /^(?:authorization|proxy[-_]?authorization|cookie|set[-_]?cookie|password|passwd|senha|token|jwt|secret|segredo|credential|credencial|api[-_]?key|access[-_]?key|private[-_]?key|supervisor[-_]?(?:pin|password|senha|proof|prova)|payload|body|request[-_]?body|response[-_]?body|response|proof|prova|result|resultado|measurement|medicao|medição|observation|observacao|observação)$/i;
const INLINE_SECRET = /\b(?:authorization|cookie|password|passwd|senha|token|jwt|secret|segredo|credential|credencial|api[-_ ]?key|access[-_ ]?key|private[-_ ]?key|supervisor[-_ ]?(?:pin|password|senha|proof|prova))\b\s*[:=]\s*(?:Bearer\s+)?[^\s,;]+/gi;
