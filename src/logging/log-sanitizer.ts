...
const INLINE_SECRET = /\b(?:authorization|cookie|password|passwd|senha|token|jwt|secret|segredo|credential|credencial|api[-_ ]?key|access[-_ ]?key|private[-_ ]?key|supervisor[-_ ]?(?:pin|password|senha|proof|prova))\b\s*[:=]\s*(?:"[^"\r\n]*"|'[^'\r\n]*'|[^\r\n,;]+)/gi;
