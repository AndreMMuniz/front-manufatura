...
const REAL_ROUTE_AUTHORIZATION = JSON.parse(
  readFileSync(
    new URL(
      '../project-specs/planodecontrole-api/examples/autorizacao-roteiros-372562-operacao-10-response.json',
      import.meta.url,
    ),
    'utf8',
  ),
...
describe('gateway Plano Controle CQ', () => {
  ...
  it('fornece SIM e NÃO para a tabela 8 quando o roteiro pendente omite as opções', async () => {
    const pendingRoute = structuredClone(REAL_PENDING_ROUTE) as {
      items: Array<{ 'ds-roteiro-pendente': { roteiro: Array<{ resultados: Array<Record<string, unknown>> }> } }>;
    };
    pendingRoute.items[0]['ds-roteiro-pendente'].roteiro[0].resultados[0] = {
  ...
  it('recusa ficha que não pertence às pendências retornadas pelo Datasul', async () => {
    ...
    const response = await fetch(`${root}/route-authorizations/route`, {
      method: 'POST',
      headers: { authorization: `Bearer ${issued.token}`, 'content-type': 'application/json' },
      body: JSON.stringify({ nrFicha: 99999, nrOrdemProducao: 372562, codOperacao: 10 }),
    });
