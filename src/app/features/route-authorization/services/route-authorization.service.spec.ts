...
describe('RouteAuthorizationService', () => {
  ...
  it('não envia identidade nem empresa ao finalizar', async () => {
    ...
    expect(api.post).toHaveBeenCalledWith('/api/quality-control/route-authorizations/finalize', {
      nrFicha: 64461,
    });
  });
  ...
  it('rejeita a identidade inválida da ficha antes de chamar o BFF', async () => {
    ...
    await expect(firstValueFrom(service.loadRoute({ ...route(), sheetNumber: 0 }, 10))).rejects.toThrow(
      'invalid-route-authorization-route',
    );
  ...
  it.each([
    ...
    [
      { kind: 'table', tableNumber: 8, optionSequence: 1 },
    ...
    expect(api.put).toHaveBeenCalledWith('/api/quality-control/route-authorizations/results', {
      nrFicha: 64462,
      codExame: 1845,
      codComponente: 1,
      ...representation,
  ...
  it('rejeita a identidade inválida do componente antes de chamar o BFF', async () => {
    ...
    await expect(
      firstValueFrom(
        service.saveComponent(64462, 0, 1, {
          kind: 'numeric',
          result: 24,
        }),
      ),
